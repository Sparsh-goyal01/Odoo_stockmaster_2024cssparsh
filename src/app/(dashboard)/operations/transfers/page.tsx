...
export default function TransfersPage() {
  ...
  const [lines, setLines] = useState<OperationLine[]>([
    { productId: 0, quantity: 1, unitOfMeasure: '', sourceLocationId: null, destinationLocationId: null, remarks: '' },
  ])
  const [submitting, setSubmitting] = useState(false)

  useEffect(() => {
    Promise.all([fetchTransfers(), fetchWarehouses(), fetchProducts()])
  }, [])
  ...
  const fetchTransfers = async () => {
    try {
      setLoading(true)
      const response = await fetch('/api/operations/transfers?limit=100')
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Button onClick={handleCreate}>New Transfer</Button>
      </div>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {transfers.map((transfer) => (
                  <tr key={transfer.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                      {transfer.sourceLocation?.name || '-'} → {transfer.destinationLocation?.name || '-'}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
